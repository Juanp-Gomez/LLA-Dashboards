...
/*select count (distinct act_acct_cd)  from  "db-analytics-prod"."tbl_fixed_cwc" where DATE_TRUNC('month', DATE(act_acct_inst_dt)) = (SELECT input_month FROM parameters)  AND DATE_TRUNC('month', DATE(dt)) = (SELECT input_month FROM parameters) and org_cntry = 'Jamaica'
...
WHERE org_cntry = 'Jamaica'
    AND act_cust_typ_nm IN ('Browse & Talk HFONE', 'Residence', 'Standard')
    AND act_acct_stat IN ('B','D','P','SN','SR','T','W') 
    AND DATE_TRUNC('month',DATE(dt)) BETWEEN ((SELECT input_month FROM parameters) - interval '6' month) AND ((SELECT input_month FROM parameters) - interval '1' month)
GROUP BY 1,2
ORDER BY 1,2
)
...
,unique_sales as (
select month, act_acct_cd from sales group by 1,2
)
...
select U.month
    ,A.month as inst_month
     ,U.act_acct_cd
        ...
        ,A.never_paid_month
        ,A.soft_dx_month
FROM unique_sales U LEFT JOIN final A ON U.act_acct_cd = A.act_acct_cd 
)
...
FROM fmc_table F LEFT JOIN final_new_cust A ON F.fixed_account = A.act_acct_cd AND F.month = A.month
)

/*select DATE_TRUNC('month',DATE(first_inst_dt)) AS f, count (distinct act_acct_cd)
from final_w_fmc
group by 1*/
-------------------------- Late Installations -----------------------
...
SELECT  F.*
        ,i.month AS sales_month
        ,DATE_TRUNC('Month',i.EndDate) AS install_month
        ,CASE WHEN installation_lapse > 5 THEN 1 ELSE 0 END AS long_install_flag
FROM final_w_fmc F LEFT JOIN service_orders_late_inst i ON F.fixed_account = cast(i.account_id as varchar(10)) AND F.month = i.month
)
...
,installations AS (
SELECT  date_trunc('Month', min(date(substring(act_cust_strt_dt,1,10)))) AS Sales_Month
        ,act_acct_cd
        ,MIN(DATE(substring(act_cust_strt_dt,1,10))) AS act_cust_strt_dt
        ,MIN(DATE(substring(act_acct_inst_dt,1,10))) AS act_acct_inst_dt
        ,date_trunc('MONTH',MIN(DATE(substring(act_acct_inst_dt,1,10)))) AS Inst_Month
FROM "db-analytics-prod"."tbl_fixed_cwc"
WHERE org_cntry = 'Jamaica'
    AND ACT_ACCT_STAT IN ('B','D','P','SN','SR','T','W')
    AND DATE(substring(act_cust_strt_dt,1,10)) BETWEEN date_trunc('Month',cast(substring(act_cust_strt_dt,1,10) AS DATE)) AND date_add('DAY',90,date_trunc('Month',CAST(substring(act_cust_strt_dt,1,10) AS DATE)))
